...
class StackableArmorReduction(StackableBuff):
    ...
    def apply_buff_to(self, champion):
        if self.stack_count < self.stack_number:
            champion.flat_armor_reduction += self.flat_reduction[self.stack_count]
            champion.percent_armor_reduction = 1 - (1 - champion.percent_armor_reduction) * (1 - self.percent_reduction[self.stack_count])
            self.stack_count += 1
