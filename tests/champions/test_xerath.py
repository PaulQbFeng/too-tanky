...
def test_dummy_0_res():
    ...
    for spell_level in range(1, 6):
        ...
        assert round(xerath.spell_q.damage(dummy)) == damage_per_level[spell_level - 1]


dummy = Dummy(1000, 30)
xerath = Xerath(
    level=16, inventory=[BlastingWand()]
)  # since xerath damage doesn't scale with his level we can fix his level


def make_test_spell(spell, values, **kwargs):
    for spell_level in range(1, len(values) + 1):
        spell.set_level(spell_level)
        assert round(spell.damage(dummy, **kwargs)) == values[spell_level - 1]
