...
def test_ignite_level_1(dummy_110):
    ...
    for i, tick in enumerate(range(1, 6)):
        assert annie.get_summoner_spell("Ignite").damage(dummy_110, number_of_ticks=tick) == dmg_champion_level_1[i]
...
def test_ignite_level_18(dummy_110):
    ...
    for i, tick in enumerate(range(1, 6)):
        assert annie.get_summoner_spell("Ignite").damage(dummy_110, number_of_ticks=tick) == dmg_champion_level_18[i]
